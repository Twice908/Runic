...
export default function MatrixView({ projectId, matrix }: MatrixViewProps) {
  ...
  return (
    <>
      ...
      {/* Matrix table */}
      <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-auto max-h-[70vh]">
        <table className="w-full text-sm border-separate border-spacing-0">
          <thead>
            <tr>
              <th className="sticky top-0 left-0 z-30 bg-gray-50 px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-600 border-b border-r border-gray-200">
                Key
              </th>
              {matrix.environments.map((env) => (
                <th
                  key={env.id}
                  className="sticky top-0 z-20 bg-gray-50 px-4 py-3 text-center text-xs font-semibold uppercase tracking-wider text-gray-600 border-b border-gray-200"
                >
          ...
          <tbody>
            {filteredRows.length === 0 ? (
              ...
              filteredRows.map((row) => (
                <tr key={row.keyName} className="hover:bg-gray-50/60">
                  <td className="sticky left-0 z-10 bg-white px-4 py-2 font-mono text-xs text-gray-800 border-b border-r border-gray-100 whitespace-nowrap">
                    {row.keyName}
                  </td>
                  {matrix.environments.map((env) => {
      ...
      <div className="absolute right-0 top-0 h-full w-full max-w-md bg-white shadow-xl overflow-y-auto">
        ...
        <div className="p-5 space-y-5">
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div>
              <p className="text-xs uppercase tracking-wider text-gray-500">Environment</p>
              <p className="mt-1 text-gray-900">{selection.envName}</p>
            </div>
            <div>
            ...
            <div className="col-span-2">
              <p className="text-xs uppercase tracking-wider text-gray-500">First seen</p>
              <p className="mt-1 text-gray-900">{relativeTime(selection.row.firstSeenAt ?? null)}</p>
            </div>
          </div>

          <div>
            <p className="text-xs uppercase tracking-wider text-gray-500 mb-2">Drift history</p>
            {loading ? (
                ...
                {history.map((ev) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <span className="font-medium text-gray-700">{ev.driftType}</span>
                      <span className="text-gray-500">{relativeTime(ev.detectedAt)}</span>
                    </div>
                    <p className="mt-0.5 text-gray-500">in {ev.environmentName}</p>
                  </li>
                ))}
              </ul>
            ) : (
              <p className="text-xs text-gray-500">No prior drift events for this key.</p>
            )}
          </div>
        </div>
      </div>
